%EEGFILT (high|low|band)-pass filter data using two-way least-squares FIR filtering
%              Multiple data channels and epochs supported.
...
% $Log: eegfilt.m,v $
% Revision 1.20  2001/2/10 Michael J. Prerau, Ph.D. - Removed ouput and fixed Nyquist error
...
min_filtorder  = 50;   % minimum filter length
...
if (1+trans)*hicutoff/nyq > 1
    hicutoff=.99*nyq/(1+trans);
    disp(['high cutoff frequency too close to Nyquist frequency, resetting to ' num2str(hicutoff)]);
end;
...
elseif locutoff > 0                % highpass filter
    ...
    %  fprintf('eegfilt() - highpass transition band width is %1.1g
    %  Hz.\n',(f(3)-f(2))*srate/2);
    m=[   0             0                   1      1];
