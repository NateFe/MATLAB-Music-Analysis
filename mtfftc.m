...
%tapers(1:end,1:end,1:end) = 1;
data_proj=data.*tapers; % product of data with tapers
%data_proj=data.*hann(length(data))*sqrt(Fs)/norm(hann(length(data)));
%data_proj=data.*sqrt(Fs)/sqrt(length(data));
%size(data_proj)
%J=(fft(data_proj,512)/Fs)*(sqrt(nfft/512));   % fft of projected data
