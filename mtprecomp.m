function [S, f]=mtprecomp(data,taperobj)
% Fast multi-taper spectrum using precomputed tapers
%
% Usage:
%
% [S, f]=mtprecomp(data,taperobj)
% Input: 
...
tapers=taperobj.tapers;
nfft=taperobj.nfft;
findx=taperobj.findx;
Fs=taperobj.Fs;

% data=change_row_to_column(data);
J=mtfftc(detrend(data),tapers,nfft,Fs);
J=J(findx,:,:);
S=squeeze(mean(conj(J).*J,2));

%ADD THIS TO OUTPUT ANGLE
%Sang=squeeze(mean(angle(J),2));
